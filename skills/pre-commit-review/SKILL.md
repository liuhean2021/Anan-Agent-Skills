...
| 🟢 **[建议]** | 可选优化，不阻塞提交，**默认自动修复（Auto-Fix）** | 直接应用修正，无需确认即可执行 |
| 💡 **[思路]** | 备选方案，供讨论 | 无需操作 |
| 📚 **[学习]** | 知识分享，无需操作 | 参考了解 |
| 🎉 **[亮点]** | 值得肯定的做法 | 保持 |

> Auto-Fix 用于 🟢 建议级的机械性问题（格式化、死代码、命名、过期注释），AI 直接应用修正并在审查文档中记录，减少开发者的修改负担。
...
**Fix-First 原则**：审查结论的同时直接处理，不只是标记。

1. **🔴 阻塞**：标记问题 + 给出可应用的修改方案。修复前使用「建设性反馈原则」引导讨论，确认后手动修复。
2. **🟡 重要**：标记问题 + 给出修改方案。修复前确认。
3. **🟢 建议**（Auto-Fix）：**直接应用修正**，不等待确认。适用场景：死代码删除、格式化、命名不规范、过时注释。
   - 执行修正后在审查报告中记录：`[AUTO-FIXED] 文件路径:行号 — 问题描述 → 修正内容`
   - 若无法安全自动修复（需人工判断），仍按 🟢 标记，不自动执行。
