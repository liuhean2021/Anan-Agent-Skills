...
第 1.5、2.5、2.6 步的产出应**合并到一份统一的「审查前置分析报告」**中，置于审查报告顶部，分节呈现。避免将每个步骤作为独立输出造成信息碎片化：
...
### 🔍 Scope Check
Scope: [CLEAN / DRIFT DETECTED / MISSING REQUIREMENTS]
Intent: ...
Delivered: ...
...
**通过**：无 🔴 阻塞问题。通过前先自查：若改动 > 100 行但结论无任何 🔴/🟡 问题，应反思是否漏审——百行以上改动无严重问题极为罕见。确认无漏审后，**必须检查是否满足 Self-Test 触发条件**（见下方），根据检查结果决定下一步：
