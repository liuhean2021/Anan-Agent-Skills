...
AI 交付 PR / 结果前，MUST 自问并如实作答，随 PR 证据包（§7.6）一并输出：

```
收尾反思：

1. 眼下最没把握的是什么？    （源自 Sam Altman）
   本实现中置信度最低、最需人工验证的部分
   ——依赖运行时才能确认的交互、未跑通的边界场景、不确定的兼容性。

2. 我最大的遗漏是什么？我没意识到什么？    （源自 Claude）
   可能被忽略但会影响结果的地方
   ——未覆盖的浏览器/设备组合、未考虑的异常态、没查过的全局影响面。
```

> 这两问不否定交付，而是标注置信度边界与盲点，把 reviewer / 用户的注意力引向最该验证的地方。没把握或遗漏为「无」时如实写「无」，禁止用「暂无风险」一类空话敷衍。
