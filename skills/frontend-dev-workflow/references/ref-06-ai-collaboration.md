...
写入前的经验判定标准（SHOULD 应用于 memory/patterns.md 与 issues.md）：
  - 一句话标准：被召回后，Agent 能否产生正向的行为变更？达不到就不写。
  - 值得沉淀的三类"Agent 自推不出"的经验（三镜头）：
      · 黑话镜头 —— 项目内部代称/缩写，字面无法推导（如"D 站"→ 盗版站点）
      · 索引镜头 —— 能力入口不在直觉路径上（如"直达页功能在 xhome 模块 FastCutXXX"）
      · 逻辑镜头 —— 反直觉工程约束（如"stop hook 不能同步上报，会阻塞进程"）
  - 写入前排除的垃圾特征（命中任一条即不写）：事实性错误 / 通用常识 / 对话摘要 /
    一次性 case / 用户主观偏好 / 缺少上下文 / 粒度混用 / 不可执行 / 证据不足
  - 涉及具体 API/类/方法时，先 `rg`/`grep` 源码验证存在性，防幻觉经验入库
  - 与历史条目去重/合并：完全重复跳过；同向有新增信息则合并；结论冲突不覆盖旧经验
  - 默认宁漏勿错：把握不足的不记录，误记一条泛泛经验比漏记干扰更大
