...
**修改页专用守护清单**（IF 页面含表单/权限/编辑态，MUST 补充以下维度）：

```
- 只读/disabled 字段：[列出条件只读字段] 在当前权限下渲染为只读状态
- 编辑态回显：[关键字段] 回显值与接口返回数据一致
- 报错后数据保留：提交失败时已填字段不丢失
- 重复提交防护：提交进行中按钮禁用，不可多次触发
- 未保存离开提示：有数据变更时路由跳转/关闭弹窗触发二次确认
```
...
- 发现校验器/mapper/权限判断等纯函数逻辑时，MUST 抽离为独立函数并补单测（Vitest/Jest），不允许只靠 E2E 守护测试覆盖

### 3.5 Step ④：精确描述改动

改动来源通常是两种情况：**有外部设计图** 或 **无外部设计图但已有锁定设计输入**。无论哪种，给 AI 的描述都必须精确。

**黄金法则：说"从 A 改到 B"，不说"改好看一点"**

> 本步骤只消费已在需求/spec 阶段锁定的 `interaction-design.md`、`design-system-context.md` 与设计引用。若设计输入缺失、失效或错误，MUST 返回对应需求/spec 阶段修正，MUST NOT 在修改页实施过程中临场补写设计文档。

#### 情况 A：有设计图

设计图可能来自 Figma、Sketch、Adobe XD、Pixso、MasterGo、即时设计、Motiff、蓝湖或其他工具。无论哪个工具，提取方式相同：

> **设计链接管理原则**：设计基线（在线链接或离线文件路径）MUST 已在需求/spec 阶段记录到 `interaction-design.md` 的「设计引用」章节（主入口）；仅维护多索引时可同步备份至 `source-links.md`。本地临时下载的截图或资产仅作需求阶段对照用，可放入 `design-assets/` 并加入 `.gitignore`，不提交 Git。
...
无外部设计图不等于无设计输入。必须先在需求/spec 阶段形成明确线框图、原型说明、截图或 UX 交互文档，并写入 `interaction-design.md`；实施阶段只按已锁定设计输入执行。
```
❌ 模糊描述（AI 会乱改）：
"表格太丑了，优化一下"
