...
本步骤只整理实施建议，不创建需求或设计文档。新增页面的前端设计文档、设计引用、线框图/原型说明必须已在需求/spec 阶段锁定；若缺失，MUST 返回对应需求/spec 阶段。
...
> **常见缺失项**（信息不足时标注 `[需确认]` 并追问）：功能模块范围、数据字段、交互方式（弹窗/跳转）、移动端处理方式、导航入口、设计基线是否完整。
