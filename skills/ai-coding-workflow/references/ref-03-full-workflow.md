...
**退出条件**：规格无歧义，checklist 已生成并闭环，规格已锁定；若涉及前端交互需求，则 Phase 2 对应的 design-level（L1/L2/L3）已判定，`interaction-design.md` 已按对应级别填写，`design-system-context.md`（或降级后的 Design System Context 章节）与设计基线已就位。
